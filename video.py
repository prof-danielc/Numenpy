...
class GameVideo:
    ...
    def render(self, logic, selected_agent=None):
        # 1. Render World
        ...
        for y in range(self.world.height):
            for x in range(self.world.width):
                ...
                # pygame.draw.rect(self.screen, (0, 0, 0), rect, 1) # border removed for smoother look
        ...
        if selected_agent:
            self._draw_brain_monitor(selected_agent, world_width_px)

    def _draw_brain_monitor(self, agent, start_x):
        # Background
        ...
        # Header
        title = font.render(f"BRAIN: {agent.agent_id} ({agent.type})", True, (255, 255, 255))
        self.screen.blit(title, (start_x + 10, y))
        y += 40

        # Drives
        self.screen.blit(font.render("DRIVES:", True, (200, 200, 255)), (start_x + 10, y))
        y += 25
        for drive, val in agent.ai.drives.drives.items():
            # Label
            self.screen.blit(font.render(f"{drive}:", True, (255, 255, 255)), (start_x + 20, y))
            # Bar
        ...
        self.screen.blit(font.render("ENERGY:", True, (255, 255, 255)), (start_x + 20, y))
        pygame.draw.rect(self.screen, (60, 60, 60), (start_x + 100, y + 4, 150, 10))
        pygame.draw.rect(self.screen, (255, 100, 100), (start_x + 100, y + 4, int(agent.energy * 150), 10))
        y += 30

        # Desires & Intention
        self.screen.blit(font.render("INTENTION:", True, (200, 255, 200)), (start_x + 10, y))
        y += 25
        intent = agent.ai.intentions.current_intention
        intent_str = f"{intent['goal']} (u={intent['utility']:.2f})" if intent else "none"
        self.screen.blit(font.render(intent_str, True, (255, 255, 100)), (start_x + 20, y))
        y += 30

        # Plan
        plan_len = len(agent.ai.planner.current_plan)
        self.screen.blit(font.render(f"PLAN STEPS: {plan_len}", True, (255, 255, 255)), (start_x + 10, y))
        y += 40

        # Traits (Personal Variance)
        self.screen.blit(font.render("TRAITS:", True, (255, 200, 100)), (start_x + 10, y))
        y += 25
        t_items = list(agent.ai.traits.traits.items())
        for i in range(0, len(t_items), 2):
            k1, v1 = t_items[i]
            col1 = f"{k1[:3]}: {v1:.2f}"
            self.screen.blit(font.render(col1, True, (200, 200, 200)), (start_x + 20, y))
            if i + 1 < len(t_items):
                k2, v2 = t_items[i+1]
                col2 = f"{k2[:3]}: {v2:.2f}"
                self.screen.blit(font.render(col2, True, (200, 200, 200)), (start_x + 140, y))
            y += 20
        y += 20

        # Learning Biases (if creature)
        if hasattr(agent.ai, 'learning') and agent.ai.learning.behavior_matrix:
            self.screen.blit(font.render("LEARNING BIASES:", True, (255, 200, 200)), (start_x + 10, y))
            y += 25
            ...
            for goal, bias in bias_items[:5]:
                self.screen.blit(font.render(f"{goal}: {bias:.2f}", True, (255, 255, 255)), (start_x + 20, y))
                y += 20
